...
func UpdateOrder() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if order.Table_id != nil {
			err := menuCollections.FindOne(ctx, bson.M{"menu_id": table.Table_id}).Decode(&table)
			defer cancel()

			if err != nil {
				c.JSON(http.StatusInternalServerError, gin.H{"error": "table id is not found"})
				defer cancel()
				return
			}
		...
		if err != nil {
			c.JSON(http.StatusBadRequest, gin.H{"error": "order item object failed"})
			defer cancel()
			return
		}
