...
func GetFoods() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		defer cancel()
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": "error occurred while querying the data"})
		}
...
func CreateFood() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if insertErr != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": "Food item not able to insert"})
			defer cancel()
			return
		}
...
func UpdateFood() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if food.Image != nil {
			updateObj = append(updateObj, bson.E{Key: "food_image", Value: &food.Image})
		}
		...
		if food.Menu_id != nil {
			...
			if err != nil {
				c.JSON(http.StatusInternalServerError, gin.H{"error": "menu id is not found"})
				defer cancel()
				return
			}
		...
		defer cancel()
		if err != nil {
			c.JSON(http.StatusBadRequest, gin.H{"error": "food item object failed"})
			return
		}
